 ...
 *
 *            Minor changes made from the `lorawan-library-for-pico`
 * implementation to support indications on confirmation
 *
 ...
 *
 * \remark Please note that when ADR is enabled the end-device should be
 * static
 */
 ...
 * Default datarate
 *
 * \remark Please note that LORAWAN_DEFAULT_DATARATE is used only when ADR
 * is disabled
 */
 ...
 *
 * \remark Please note that ETSI mandates duty cycled transmissions. Use
...
static void OnSysTimeUpdate( bool    isSynchronized,
 ...
 *
 * \warning If variable is equal to 0 then the MCU can be set in low power
 * mode
 */
...
const char* lorawan_default_dev_eui( char* dev_eui )
{
  ...
  sprintf( dev_eui, "%02X%02X%02X%02X%02X%02X%02X%02X", boardId[0],
           boardId[1], boardId[2], boardId[3], boardId[4], boardId[5],
           boardId[6], boardId[7] );

  return dev_eui;
}

int lorawan_init( const struct lorawan_sx1276_settings* sx1276_settings,
                  LoRaMacRegion_t                       region )
{
  ...
  SpiInit( &SX1276.Spi,
           ...
           sx1276_settings->spi.sck /*SCK*/, NC );
...
int lorawan_send_unconfirmed( const void* data, uint8_t data_len,
                              uint8_t app_port )
{
  ...
  if ( LmHandlerSend( &appData, LORAMAC_HANDLER_UNCONFIRMED_MSG ) !=
       LORAMAC_HANDLER_SUCCESS ) {
    return -1;
  }
...
static void OnNetworkParametersChange( CommissioningParams_t* params )
{
  ...
  if ( AbpSettings != NULL ) {
    ...
    // Tell the MAC layer which network server version are we connecting
    // too.
...
static void OnTxData( LmHandlerTxParams_t* params )
{
  ...
  printf( "Got here - %d %d %d\n", params->AppData.BufferSize,
          params->MsgType == LORAMAC_HANDLER_CONFIRMED_MSG,
          params->AckReceived );
...
static void OnClassChange( DeviceClass_t deviceClass )
{
  ...
  // Inform the server as soon as possible that the end-device has
  // switched to ClassB
  LmHandlerAppData_t appData = {
      .Buffer     = NULL,
      .BufferSize = 0,
